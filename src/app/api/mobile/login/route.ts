...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = userLoginSchema.safeParse({ email: body.email, password: body.password });
    if (!parsed.success) {
    ...
    const accessToken = await signAccessToken({ sub: user.id, role: user.role, hotelId: user.hotelId });
